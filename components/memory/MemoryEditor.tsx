...
export function MemoryEditor({ entry, onUpdate, onDelete, localOnly, onBusy }: Props) {
  ...
  const addPhoto = async (file: File) => {
    ...
    try {
      ...
      const planet: NonNullable<JournalEntry['planet']> = { version: 1, seed: current.planet?.seed ?? seedOf(id), palette: palette || current.planet?.palette || ['#e5ef70', '#ed8eaa', '#5168cf'], cover: dataURL, material: 'resin', atmosphere: current.planet?.atmosphere };
  ...
  return <section className="memory-editor">
    ...
    <div className="photo-strip" onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); if (e.dataTransfer.files[0]) void addPhoto(e.dataTransfer.files[0]); }}>
      {images.map((src, i) => <button key={src} className={`photo-tile ${imageOf(entry) === src ? 'active' : ''}`} title="设为星球照片" onClick={() => onUpdate(entry.id, { planet: { version: 1, seed: entry.planet?.seed ?? seedOf(entry.id), palette: entry.planet?.palette || ['#e5ef70', '#ed8eaa', '#5168cf'], material: 'resin', cover: src, atmosphere: entry.planet?.atmosphere } })}><img src={src} alt={`回忆照片 ${i + 1}`} /><span>{imageOf(entry) === src ? '星球封面' : '设为封面'}</span></button>)}
    ...
    <details className="editor-drawer">
      ...
      <div className="memory-attributes"><label>这一天的心情<select value={entry.userMood || ''} onChange={e => onUpdate(entry.id, { userMood: e.target.value })}><option value="">先不定义</option>{['平静', '开心', '思念', '疲惫', '复杂', '兴奋'].map(m => <option key={m}>{m}</option>)}</select></label><label>记忆里的天气<select value={entry.weather || 'none'} onChange={e => onUpdate(entry.id, { weather: e.target.value as JournalEntry['weather'] })}><option value="none">未记录</option><option value="clear">晴</option><option value="rain">雨</option><option value="cloud">雾与云</option><option value="snow">雪</option></select></label><label>网页背景<select value={entry.planet?.atmosphere || ''} onChange={e => onUpdate(entry.id, { planet: { version: 1, seed: entry.planet?.seed ?? seedOf(entry.id), palette: entry.planet?.palette || ['#e5ef70', '#ed8eaa', '#5168cf'], material: entry.planet?.material || 'resin', cover: entry.planet?.cover, atmosphere: (e.target.value || undefined) as NonNullable<JournalEntry['planet']>['atmosphere'] } })}><option value="">随天气自动变化</option><option value="ocean">海面</option><option value="sky">蓝天</option><option value="aurora">极光</option><option value="stars">星空</option><option value="cosmos">宇宙</option><option value="clouds">白云与雾</option><option value="rain">雨夜</option></select></label></div>
