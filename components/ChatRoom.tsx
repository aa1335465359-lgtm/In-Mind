...
import { ArrowLeft, ArrowDown, Radio, Shuffle, Send, Flame, Link, LogOut, LockKeyhole, BookOpen, X, Reply, LoaderCircle, Users } from 'lucide-react';
...
function Message({ msg, isMe, onReply, onView, onExpire }: { msg: ChatMessage; isMe: boolean; onReply: () => void; onView: () => void; onExpire: () => void }) {
  ...
  const expiresAt = msg.expiresAt || msg.timestamp + 60_000;
  const [remaining, setRemaining] = useState(() => ephemeralRemaining(expiresAt));

  useEffect(() => {
    if (!msg.isEphemeral) return;
    const tick = () => {
      const next = ephemeralRemaining(expiresAt); setRemaining(next);
      if (next === 0 && !expired.current) { expired.current = true; callback.current(); }
    };
    tick(); const timer = window.setInterval(tick, 500);
    return () => window.clearInterval(timer);
  }, [msg.isEphemeral, expiresAt]);

  if (msg.type === 'system' || msg.type === 'screenshot-alert') return <div className="chat-system">{msg.content}</div>;
  return <article className={`chat-message ${isMe ? 'mine' : ''} ${msg.isEphemeral ? 'is-burning' : ''}`} style={{ '--sender-hue': senderHue(msg.senderId) } as React.CSSProperties}>
    ...
    <div className="message-body">
      ...
      <div className={`message-bubble ${msg.isEphemeral ? 'ephemeral-open' : ''}`}>
      ...
      <div className="message-actions">{msg.isEphemeral && <span className="burn-status" style={{ '--burn-progress': remaining / 60 } as React.CSSProperties}><i aria-hidden="true" /><Flame size={12} />{remaining}s 后自动消失</span>}<button aria-label="引用回复" onClick={onReply}><Reply size={13} /> 回复</button></div>
    </div>
  </article>;
}
  ...
  return <section className="radio-room">
    ...
    <div className="radio-content">
      ...
      {session.connection !== 'joined' ? <div className="radio-join"><div className="frequency-number">001<span>PUBLIC FREQUENCY</span></div><div className="join-card"><span className="eyebrow">YOUR TEMPORARY IDENTITY</span><h2>{session.connection === 'connecting' ? '正在接入频率…' : '先挑一个荒谬的代号'}</h2><label><span>匿名代号</span><div><input value={name} onChange={event => setName(event.target.value)} maxLength={28} onKeyDown={event => { if (event.key === 'Enter' && !event.nativeEvent.isComposing) void join(); }} /><button aria-label="换一个随机代号" onClick={shuffleName}><Shuffle size={16} /> 换一个</button></div></label><p className="nickname-example">例如：爱吃鸡蛋的捣蛋鬼1、下班失败的奥特曼37</p><button className="primary-button" disabled={session.connection === 'connecting'} onClick={() => void join()}>{session.connection === 'connecting' ? <LoaderCircle size={16} className="spin" /> : <Radio size={16} />}{session.connection === 'error' ? '重新连接' : '带着这个代号进入'}</button>{session.error && <p className="inline-notice" role="alert">{session.error}</p>}</div></div> : <>
        ...
        <div className={`chat-composer ${burn ? 'burn-mode' : ''}`}>{replying && <div className="reply-preview"><span>回复 {replying.senderName}：{replying.isEphemeral ? '[60 秒即焚消息]' : replying.content.slice(0, 60)}</span><button aria-label="取消回复" onClick={() => setReplying(null)}><X size={14} /></button></div>}<textarea ref={textarea} value={draft} onChange={event => setDraft(event.target.value)} placeholder={burn ? '发送后开始计时，60 秒后自动消失…' : '说点什么，不必留下真实名字…'} aria-label="聊天消息" maxLength={2000} rows={1} onFocus={() => requestAnimationFrame(() => scrollToBottom())} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); void send(); } }} /><div className="composer-tools"><div><button className={burn ? 'burn-active' : ''} onClick={() => setBurn(value => !value)} aria-pressed={burn}><Flame size={15} /> {burn ? '60 秒即焚' : '定时即焚'}</button><button onClick={() => setShowShare(value => !value)}><BookOpen size={15} /> 分享一页</button></div><button className="send-button" onClick={() => void send()} disabled={sending || !draft.trim()} aria-label="发送消息">{sending ? <LoaderCircle size={16} className="spin" /> : <Send size={16} />}</button></div>{showShare && <div className="share-picker"><select aria-label="选择分享的回忆" value={share} onChange={event => setShare(event.target.value)}><option value="">选择一段回忆（仅分享文字）</option>{entries.map(entry => <option key={entry.id} value={entry.id}>{titleOf(entry)}</option>)}</select><button disabled={!share || sending} onClick={() => void shareEntry()}>确认分享</button></div>}<div className="composer-hint"><span>{burn ? '从发送时开始计时 · 切到后台仍会过期' : 'Enter 发送 · Shift + Enter 换行'}</span><span>{draft.length}/2000</span></div></div>
      </>}
