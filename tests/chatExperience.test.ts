...
import { EPHEMERAL_LIFETIME_MS, ephemeralExpired, ephemeralRemaining } from '../services/ephemeralMessage';
...
test('ephemeral countdown begins at an explicit open time and ends after 60 seconds', () => {
  const openedAt = 1_000_000;
  assert.equal(EPHEMERAL_LIFETIME_MS, 60_000);
  assert.equal(ephemeralRemaining(openedAt, openedAt), 60);
  assert.equal(ephemeralRemaining(openedAt, openedAt + 30_001), 30);
  assert.equal(ephemeralExpired(openedAt, openedAt + 59_999), false);
  assert.equal(ephemeralExpired(openedAt, openedAt + 60_000), true);
});
...
test('chat layout keeps the composer outside the only scrolling region', () => {
  ...
  assert.match(component, /点击打开，随后保留 60 秒/);
